...
//		System.out.println("ПОДДЕРЖИВАЮТСЯ АРАБСКИЕ И РИМСКИЕ ЦИФРЫ.");
		...
		for (String $a : $iDigits) {
			if ($iDigits.length > 2) {
				System.out.println("Введены НЕПРАВИЛЬНЫЕ ОПЕРАНДЫ."); // ПРОВЕРКА НА НЕ БОЛЕЕ 2 ОПЕРАНДОВ ОДНОЙ СИСТЕМЫ.
				System.exit(0);
			}
			$a = $a.trim();
...
//					System.out.println("РИМСКАЯ цифра. " + ($b + 1));
					$romanOperands[$romanCounter] = ++$b; $romanCounter++;
				} else if($b < 10 && $a.equals($arabicNumbers[$b])) {
//					System.out.println($a.trim());
//					System.out.println("АРАБСКАЯ цифра. " + ($b + 1));
					$arabicOperands[$arabicCounter] = ++$b; $arabicCounter++;
				}
			}
		}
...
//		System.out.println("РИМСКИХ цифр: " + $romanCounter + " шт.");
		if ($arabicCounter == 2 && $iDigits.length == 2) { // ПРОВЕРКА НА ОДИНАКОВЫЕ ЦИФРЫ И ПРОВЕРКА НА НЕ БОЛЕЕ 2 ОПЕРАНДОВ РАЗНЫХ СИСТЕМ.
			if ($scanner.contains("+")) System.out.println("РЕЗУЛЬТАТ СЛОЖЕНИЯ: " + ($arabicOperands[0] + $arabicOperands[1]));
			if ($scanner.contains("-")) System.out.println("РЕЗУЛЬТАТ ВЫЧИТАНИЯ: " + ($arabicOperands[0] - $arabicOperands[1]));
			if ($scanner.contains("*")) System.out.println("РЕЗУЛЬТАТ УМНОЖЕНИЯ: " + ($arabicOperands[0] * $arabicOperands[1]));
			if ($scanner.contains("/")) System.out.println("РЕЗУЛЬТАТ ДЕЛЕНИЯ: " + ($arabicOperands[0] / $arabicOperands[1]));
			...
			if ($scanner.contains("+")) {
				...
				} else System.out.println("Введены НЕПРАВИЛЬНЫЕ ОПЕРАНДЫ.");
			}
			if ($scanner.contains("-")) {
				$resultRoman = $romanOperands[0] - $romanOperands[1];
				if($resultRoman > 0) {
